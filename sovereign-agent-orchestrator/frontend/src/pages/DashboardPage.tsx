...
function DashboardPage() {
  ...
  const metrics = role === 'admin' ? [
    { label: 'Recent jobs', value: jobs.length, note: 'Latest 100 accessible runs', tone: 'accent' as const },
    { label: 'Success rate', value: finished ? `${Math.round(done / finished * 100)}%` : '—', note: 'Of completed or failed runs', tone: 'success' as const },
    { label: 'Active / queued', value: active, note: `${review} awaiting approval`, tone: 'info' as const },
    { label: 'Knowledge sources', value: files.length, note: `${outputCount} artifacts from recent runs`, tone: 'warning' as const },
  ] : role === 'higher' ? [
    { label: 'Pending approvals', value: review, note: 'Among your accessible runs', tone: 'warning' as const },
    { label: 'Active jobs', value: active, note: 'In progress now', tone: 'info' as const },
    { label: 'Verified', value: verified, note: 'Recent verification passes', tone: 'success' as const },
    { label: 'Your uploads', value: files.length, note: 'Documents you added', tone: 'accent' as const },
  ] : [
    { label: 'Your sessions', value: conversations.length, note: 'Accessible conversations', tone: 'accent' as const },
    { label: 'Active jobs', value: active, note: 'Currently in progress', tone: 'info' as const },
    { label: 'Recent outputs', value: outputCount, note: 'Artifacts in recent runs', tone: 'success' as const },
    { label: 'Your uploads', value: files.length, note: 'Documents you added', tone: 'warning' as const },
  ]
  ...
  return <main className="app-page min-h-0 flex-1 overflow-y-auto px-4 py-7 sm:px-8 sm:py-9 xl:px-12">
    <div className="mx-auto max-w-[1400px] pb-12">
      ...
      <div className="mt-10 grid items-start gap-8 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.8fr)]">
        <div className="min-w-0"><ProtocolPanel job={detail} summary={latest} loading={jobsLoading || detailLoading} /></div>
        <div className="flex min-w-0 flex-col gap-4"><ScopeCard role={role} files={files} conversations={conversations} jobs={jobs} /><ActivityCard role={role} jobs={jobs} loading={jobsLoading} /><div className="work-panel border-l-2 border-l-accent/65 p-5"><p className="label-micro text-accent">Next step</p><p className="mt-2 text-sm font-medium text-foreground">{role === 'admin' ? 'Keep operations in view.' : role === 'higher' ? 'Inspect pending approvals.' : 'Continue your work.'}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">{role === 'admin' ? 'Inspect recent tasks or add new knowledge to the workspace.' : role === 'higher' ? 'Check approval requests visible to your account and inspect their verification details.' : 'Start a new session or return to your latest conversation.'}</p><Link to={role === 'higher' ? '/app/tasks?filter=review' : role === 'admin' ? '/app/knowledge/ingest' : conversations[0] ? `/app/feed/${conversations[0].id}` : '/app/feed'} className="mt-4 inline-block text-xs font-medium text-accent hover:underline">{role === 'admin' ? 'Manage knowledge' : role === 'higher' ? 'View pending tasks' : 'Continue to feed'} →</Link></div></div>
      </div>
    </div>
  </main>
}
