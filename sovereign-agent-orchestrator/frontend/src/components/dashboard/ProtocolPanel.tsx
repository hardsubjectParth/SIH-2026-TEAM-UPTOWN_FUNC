  ...
  return (
    <section aria-labelledby="last-run-title" className="min-w-0">
      ...
      <div className="protocol-surface p-5 sm:p-6">
        ...
        {!job ? <p className="mb-5 rounded-lg border border-dashed border-rule px-3 py-2 text-xs text-muted-foreground">{loading ? 'Loading protocol details…' : summary ? 'Protocol details are unavailable right now.' : 'No runs yet. Start a task to populate this pipeline.'}</p> : null}
        <ol className="space-y-0">
          {STEPS.map((step, index) => {
            ...
            return <li key={step.title} className="flex gap-3.5">
              <div className="flex w-5 shrink-0 flex-col items-center"><span className={`flex size-5 shrink-0 items-center justify-center rounded-full border text-[11px] ${complete ? 'border-success/40 bg-success/10 text-success' : current ? 'border-info/70 bg-info/10 text-info' : 'border-rule-strong text-muted-foreground'}`} aria-label={complete ? 'Complete' : current ? 'In progress' : 'Pending'}>{complete ? '✓' : current ? '•' : ''}</span>{index < STEPS.length - 1 ? <span className="my-1 min-h-6 w-px flex-1 bg-fill-strong" /> : null}</div>
